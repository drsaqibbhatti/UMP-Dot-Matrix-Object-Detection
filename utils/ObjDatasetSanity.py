...
import csv
class ObjDatasetSanity(Dataset):
    def __init__(self, root, transform=None, useHFlip=False, useVFlip=False, img_size=(512, 512), normalize_boxes=True,datatype="train"):
        """
        Args:
            root (str): Root path like "/path/to/Inner_LT"
            datatype (str): "train" or "val"/"validation"
        """
        assert datatype.lower() in ("train", "val", "validation"), "Invalid datatype"
        split_folder = "Train" if "train" in datatype.lower() else "Validation"
        
        self.root = os.path.join(root, split_folder)
        ...
        print(f"[INFO] Collected {len(self.image_files)} images from {split_folder} folder.")
