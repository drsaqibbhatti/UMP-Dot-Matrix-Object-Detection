...
class Head(torch.nn.Module):
    ...
    def initialize_biases(self):
        ...
        for a, b, s in zip(m.box, m.cls, m.stride):
            ...
            # cls (.01 objects, 80 classes, 512 img)
            b[-1].bias.data[:m.nc] = math.log(5 / m.nc / (512 / s) ** 2)


class ObjModel(torch.nn.Module):
...
def ObjNano(num_classes: int = 80):
    depth = [1, 1, 1]
    width = [3, 16, 32, 64, 128, 256]
    return ObjModel(width, depth, num_classes)


def ObjSmall(num_classes: int = 80):
    depth = [1, 2, 2]
    width = [3, 16, 32, 64, 128, 256]
    return ObjModel(width, depth, num_classes)


def ObjMedium(num_classes: int = 80):
    depth = [1, 2, 2]
    width = [3, 32, 64, 128, 256, 256]
    return ObjModel(width, depth, num_classes)


def ObjLarge(num_classes: int = 80):
    depth = [1, 2, 2]
