...
from utils.util import ComputeLoss, EMA, pixel_iou_from_nms, clip_gradients, setup_seed, visualize_predictions
...
def run_train(train_path, val_path,result_model_dir_path=None, imageHeight=int(320), imageWidth=int(320), data_type="default", resume_training_path=None, hyperparam_path=None):
    ...
    validation_enabled = bool(val_path and all(os.path.isdir(p) for p in val_path))
    
    # Transformationshow 
    transformAug = transforms.Compose([
        transforms.ColorJitter(brightness=(0.35, 1.1), contrast=(0.8, 1.2)),
    ...
    if os.path.exists(hyperparam_path):
        try:
            ...
            trainBatchSize = data.get("BatchSize", trainBatchSize)
            learningRate = data.get("LearningRate", learningRate)
            target_iou = data.get("TargetAccuracy", target_iou)
    ...
    print(f" 하이퍼 파라메터 정보를 로드했습니다. Epochs= {epochs}, BatchSize= {trainBatchSize}, LearningRate= {learningRate}, TargetAccuracy= {target_iou}")
    ...
    if resume_training_path:
        if os.path.exists(resume_training_path):
            model.load_state_dict(torch.load(resume_training_path, map_location=device))
            start_epoch = 0  # Start fresh training from 0
            logging.info(f"Loaded model weights from: {resume_training_path}.")
            flush_stdout()
        else:
            raise FileNotFoundError(f"Model file not found: {resume_training_path}")
    else:
        logging.info("No pretrained model provided. Starting training from scratch.")
        flush_stdout()
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if validation_enabled:
            with torch.no_grad():
                for i, (images_val, targets_val) in enumerate(validLoader):
                    ...
                    mean_iou, val_per_class_totals, val_per_class_counts = pixel_iou_from_nms(
                        outputs=outputs,
                        targets=targets_val,
                        width=width,
                        height=height,
        ...
        if validation_enabled:
            logging.info(f"[Epoch {epoch}] Train Loss: {train_loss:.4f}, Train IoU: {train_mean_iou:.4f}, Validation IoU: {overall_iou:.4f}")
            flush_stdout()
        else:
            logging.info(f"[Epoch {epoch}] Train Loss: {train_loss:.4f}, Train IoU: {train_mean_iou:.4f}")
            flush_stdout()
        ...
        for cls, iou_total in val_class_iou_totals.items():
            ...
            class_name = LABEL_NAMES.get(cls, f"Class_{cls}")
            per_class_iou[f"Validation {class_name} IoU"] = round(class_mean_iou, 4)
...
def plot_metrics(metrics_df, Base_dir, epoch):
    ...
    if 'ValidIoU' in metrics_df.columns:
        plt.plot(metrics_df['Epoch'], metrics_df['ValidIoU'], label='ValidIoU')

        for col in metrics_df.columns:
            if "Validation" in col and "IoU" in col:  # Identify class IoU columns
